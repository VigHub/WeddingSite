<script lang="ts">
	...
	const screenColumns = images.length % 4 === 0 ? 4 : 3;
...
<section
	class="grid grid-cols-{mobileColumns} md:grid-cols-{screenColumns} gap-2 p-2 variant-ghost-tertiary rounded-xl"
>
	{#each range(0, images.length, 3) as i}
		<div class="grid gap-2">
			{#each range(i, i + 3, 1) as j}
				<div>
					<!-- svelte-ignore a11y-click-events-have-key-events -->
					<img class="h-full max-w-full rounded-lg" src={images[j]} alt="" />
				</div>
			{/each}
		</div>
	{/each}
